        ...
        for result in results:
            for obj in result.boxes.data:
                ...
                if confidence > self.confidence_threshold:
                    self.memory[class_name] = min(self.max, self.memory[class_name] + self.increment * (confidence**2))
                elif confidence < self.forgetting_threshold:
                    self.memory[class_name] = max(0.0, self.memory[class_name] - self.decrement * confidence)

            for class_name in list(self.memory.keys()):
                if class_name not in detected_objects:
                   self.memory[class_name] = max(0.0, self.memory[class_name] - self.decrement)
